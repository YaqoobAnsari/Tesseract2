%% Tesseract++: Journal Extension (ACM TSAS) - REVISION 1, coloured working copy
...
%% Uncoloured new text = editorial improvements not tied to a reviewer.
...
\input{.revision_macros_clean}   % per-reviewer revision colouring (see README)
...
\begin{enumerate}
    \item \textbf{Text annotations}. Rooms and spaces carry text labels, and \rtwo{semantic classes are seeded exclusively by these labels rather than by fill colours}. Table~\ref{tab:vocabulary} lists the recognized vocabulary.
...
\begin{table}[t]
\centering
\caption{\rtwo{Label vocabulary. Interpretation is case-insensitive, and every semantic class must be present for door classification to proceed.}}
...
\rone{Recognition output is normalized by semantic context rather than by generic spell correction. Tokens are matched case-insensitively against the closed label vocabulary of Table~\ref{tab:vocabulary}, so a vocabulary word must be recognized exactly after case folding. Room identifiers are expected to be numeric, so alphabetic look-alikes inside otherwise numeric tokens are mapped to digits, which resolves confusions such as the letter O for the digit zero.}
...
\rone{We state the guarantee precisely because it differs from the conference phrasing. Paths are not forced through the main node. A subnode adjacent to a door exits directly through the closest-node edge $(c_d, d)$, and on our primary evaluation floor 69 of 186 subnode-to-door shortest paths bypass the main node. This behaviour is intentional. The property downstream routing needs is completeness at linear cost, and the shortcut edge strictly improves realized path lengths. Section~\ref{subsec:graph_eval} quantifies the trade-off with a three-way ablation.}
...
\rthree{Inter-floor edges carry modeled vertical travel instead of a nominal cost. The weight is the product of a travel factor, the storey height, and the drawing scale in pixels per metre. The scale is bootstrapped from the median short side of detected door boxes under the standardized 0.9-metre leaf width, the storey height defaults to 4 metres, and the travel factor is 2.2 for stairs and 1.0 for elevators. The estimated scales are independently consistent across floors of our evaluation building, 31.1 versus 32.2 pixels per metre, which supports the bootstrap. When no scale can be estimated the system falls back to a unit weight and emits a warning.}
...
\caption{First-floor section, $1946 \times 1574$ pixels.}
\end{subfigure}
\hfill
\begin{subfigure}{0.40\textwidth}
\includegraphics[width=\linewidth]{figures/dataset_gf.png}
\caption{Ground-floor section, $1182 \times 1105$ pixels.}
\end{subfigure}
\caption{\rone{Representative evaluation drawings. Rooms carry numeric labels, corridors carry hall labels, outdoor regions carry na labels, and stairs and elevators are labeled at their shafts. Walls are dark strokes and doors use standard arc symbols.}}
\label{fig:dataset}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{figures/pruning_comparison.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{figures/corridor_density.png}
\caption{\rone{Corridor density trade-off on first-floor section A. Completeness holds from 20 to 60 pixels and collapses at 80, while graph size grows steeply toward dense spacing. Annotations give the mean route error against the 20-pixel reference.}}
\label{fig:density}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{figures/stacked_floors.png}
\caption{Stacked visualization of a merged two-floor graph. Vertical links connect matched transition shafts.}
\label{fig:stacked}
\end{figure}
...
Wall masks for registration are downsampled to 400 pixels on the long side, and connected components below 30 square pixels are removed as glyphs. Alignment counts a pair within 2\% of the image diagonal. Plausibility gates accept scales between 0.4 and 2.5 and rotations up to 60 degrees, and chamfer scoring requires at least 25\% mutual coverage. The rejection price is 0.10, and the cost weights are 1.0 for position, 1.0 for kind mismatch, and 0.05 for graph context. Inter-floor weights use a 0.9-metre door leaf, a 4-metre storey height, and travel factors of 2.2 for stairs and 1.0 for elevators.

\section{Additional Visualizations}
\label{app:visualizations}

Figures~\ref{fig:app_ff} and~\ref{fig:app_gf} reproduce the evaluation drawings of Figure~\ref{fig:dataset} at full page width so that labels, door symbols, and wall conventions are legible.
